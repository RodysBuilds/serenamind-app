...
## V11 mobiele registratie
Profiel maken werkt nu onafhankelijk van het tijdstip waarop DOMContentLoaded afgaat, met touchbediening, zichtbare validatiefouten, opslagcontrole, dubbele-klikblokkering, veilige HTTPS-controle en cache-busting.

## V12 auth-overlay fix
De inloglaag wordt nu na succesvolle registratie of login op vijf manieren gesloten: hidden-attribuut, authenticated-body-status, display:none, pointer-events:none en inert. Daarnaast worden oude dubbele click/touch handlers vervangen door één definitieve handler en wordt de schermstatus bij pageshow hersteld.

## V13 Stable cross-platform auth
Een zelfstandig auth-core.js bestuurt registratie en login, zelfs wanneer een optionele functie in app.js een runtimefout geeft. Dit voorkomt dat de inloglaag blijft hangen. Getest op syntaxis en ontworpen voor Safari/iOS PWA, Chrome/Android, Edge/Windows en desktop Safari/Chrome.
